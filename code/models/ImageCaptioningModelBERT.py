...
class ImageCaptioningModelBERT:
    def __init__(self, processor, embedding_matrix, image_features, captions, search="Greedy"):
        ...
        self.bert_layer.resize_token_embeddings(len(self.tokenizer))
    ...
    def transformer_block(self,  inputs):
        # MultiHeadAttention
        attention_output = MultiHeadAttention(num_heads=8, key_dim=self.embedding_dim)(inputs, inputs)
        attention_output = layers.Dropout(0.5)(attention_output)
        out1 = layers.LayerNormalization(epsilon=1e-6)(inputs + attention_output)
        ffn_output = layers.Dense(4 * self.embedding_dim, activation="relu")(out1)
        ffn_output = layers.Dense(self.embedding_dim)(ffn_output)
        ffn_output = layers.Dropout(0.5)(ffn_output)
    ...
    def build_model(self):
        ...
        # Embed the caption input using the BERT layer
        embedded_caption = self.bert_layer(caption_input)[0]

        # Project the image features into the same dimension as the embedded captions
        image_features_dense = Dense(self.bert_layer.output_shape[-1], activation="relu")(image_input)
        ...
        # Define the first Transformer layer
        transformer_layer1 = MultiHeadAttention(num_heads=8, key_dim=self.bert_layer.output_shape[-1])

        # Apply the first Transformer layer to the image-caption input
        transformer_output1 = transformer_layer1(image_caption_input, image_caption_input)

        # Define the second Transformer layer
        transformer_layer2 = MultiHeadAttention(num_heads=8, key_dim=self.bert_layer.output_shape[-1])

        # Apply the second Transformer layer to the output of the first Transformer layer
        transformer_output2 = transformer_layer2(transformer_output1, transformer_output1)

        # Define the output layer
        output_layer = TimeDistributed(Dense(self.vocab_size, activation="softmax"))

        # Connect the output layer to the Transformer output
        output = output_layer(transformer_output2)
