...
from tensorflow.keras.layers import Input, Embedding, LSTM, Dense, Dropout, Add, GRU
...
class ImageCaptioningModelNoTransformLSTM:
    def __init__(self, processor, embedding_matrix, image_features, captions, search="Greedy"):
        ...
        self.rnn_layer_dims = [self.vocab_size, 1024, 528, 256]
        self.search = search
    ...
    def build_model(self):
        ...
        gru_layers = []
        for i in range(len(self.rnn_layer_dims)):
            gru_layers.append(GRU(self.rnn_layer_dims[i], return_sequences=True))
            gru_layers.append(Dropout(0.5))
        
        # Apply the GRU layers to the image-caption input
        rnn_output = image_caption_input
        for gru_layer in gru_layers:
            rnn_output = gru_layer(rnn_output)
